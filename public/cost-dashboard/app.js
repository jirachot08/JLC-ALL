...
async function fetchBudgetData(platform) {
    ...
    const methods = [
        // Method 1: Server-side API proxy (no CORS issues)
        async () => {
            const apiUrl = `/api/cost-sheets?gid=${monthSheet.gid}`;
            const response = await fetchWithTimeout(apiUrl, 15000);
            if (!response.ok) throw new Error('API proxy fetch failed');
...
async function fetchSheetData(platform) {
    ...
    const methods = [
        // Method 1: Server-side API proxy (no CORS issues)
        async () => {
            const apiUrl = `/api/cost-sheets?gid=${sheet.gid}`;
            const response = await fetchWithTimeout(apiUrl, 15000);
            if (!response.ok) throw new Error('API proxy fetch failed');
            return await response.text();
        },
...
function parseCSVData(csvText) {
    ...
    // Find header row: look for 'วันที่' or 'Date' that has actual column headers (not empty)
    let headerRowIndex = -1;
    let dataStartIndex = -1;

    for (let i = 0; i < lines.length; i++) {
        if (lines[i][0] === 'วันที่' || lines[i][0] === 'Date') {
            const hasHeaders = lines[i].slice(1).some(h => h && h.trim() !== '');
            if (hasHeaders) {
                headerRowIndex = i;
                dataStartIndex = i + 1;
                break;
